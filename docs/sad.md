...
| `setUserModulesAction`  | Admin         | Set `moduleWork` and/or `moduleFinance` flags for a user. Cannot remove own module access. |
...
erDiagram
    USER {
        ...
        boolean moduleWork "default true"
        boolean moduleFinance "default true"
...
#### 10.3.0 Module Access Check

Module licensing (BRD-001 FR-UG-05) adds a third access dimension alongside authentication and role. `moduleWork` and `moduleFinance` are boolean flags on the User model, included in the JWT payload and `session.user`.

The check is applied in `app/(app)/layout.tsx` (for Work routes) and `app/(app)/finance/layout.tsx` (for Finance routes):
...
The Sidebar renders navigation groups conditionally:
```typescript
// Only render Finance nav group if user has Finance access
{session.user.moduleFinance && <FinanceNavGroup collapsed={collapsed} />}
// Only render Work nav group if user has Work access
{session.user.moduleWork && <WorkNavGroup collapsed={collapsed} />}
```

Module flags are included in the NextAuth `session` callback alongside `role` and `id`:
```typescript
callbacks: {
  session({ session, token }) {
    session.user.id = token.id;
    session.user.role = token.role;
    session.user.moduleWork = token.moduleWork;       // added
    session.user.moduleFinance = token.moduleFinance; // added
    return session;
  }
}
```

`setUserModulesAction` in `lib/actions/admin.ts` updates the flags and calls `revalidatePath('/admin/users')`. The user's JWT is refreshed on their next request (NextAuth re-reads from DB on session refresh cycle), so changes take effect within the session window without requiring re-login.
...
| 1.3     | 2026-03-06 | Tech Lead         | Added `moduleWork`/`moduleFinance` boolean flags to User ER model; added `setUserModulesAction` to server actions table; added §10.3.0 Module Access Check (JWT payload, layout guards, conditional Sidebar rendering, session callback); updated BRD reference to v1.3. |
