...
#### FR-T-05 — Kanban Board View

**User Story:**
As an employee, I want to view my tasks on a Kanban board so that I can see progress by status at a glance.

**Acceptance Criteria:**
- AC-T-05-1: A board view at `/tasks/board` shows tasks grouped into columns by status (`Backlog`, `In Progress`, `Blocked`, `In Review`, `Done`).
- AC-T-05-2: "New task" initiated from the board view (`/tasks/new?from=board`) redirects back to `/tasks/board` after successful creation, preserving board context. The cancel action also returns to `/tasks/board` when initiated from the board.
- AC-T-05-3: The task list view and the board view are independently navigable; switching between them does not alter task data.

---

### 5.2 Work Log

**Overview:** Work logs record effort and progress associated with a task. They serve as the source of truth for what was done and when. Where a user operates as both an employee and a sole trader (see FR-UG-06), each log entry carries a **Work Context** tag to indicate which capacity the effort relates to. This context drives timesheet billing and expense segregation in the Finance module.
...
**User Story:**
As a user, I want to log work against a task so that I have a detailed record of the effort spent, attributed to the correct employment context.
...
- AC-WL-01-6: If the user's Employment Profile (FR-UG-06) is set to **Both** (employee and sole trader), a mandatory **Work Context** selector is shown: `Day Job` or `Sole Trading`. The selected value is stored with the entry.
- AC-WL-01-7: If the user's Employment Profile is **Employed** or **Sole Trader** only, the Work Context field is not shown (it is inferred from the profile and stored automatically).
- AC-WL-01-8: Work Context values displayed to the user are `Day Job` (stored as `EMPLOYED`) and `Sole Trading` (stored as `SOLE_TRADER`).
...
**User Story:**
As a user, I want to see all my work log entries in a consolidated view so that I can review effort across all tasks and contexts.
...
- AC-WL-03-4: If the user's Employment Profile is **Both**, an additional **Work Context** filter is shown: `All`, `Day Job`, `Sole Trading`. The summary totals update to reflect the filtered context.
...
- AC-UG-01-10: A notification badge is displayed in the application top bar (visible to Admin users only) showing the count of pending user registrations (`isActive = false`, `rejectedAt = null`). Clicking the badge navigates to `/admin/users`. The badge is hidden when the count is zero.
...
- AC-UG-03-3: **Admin:** Full read/write access to all data; access to the Admin panel (User and Group management, system configuration). An Admin navigation link (shield icon) is visible in the Sidebar only for Admin-role users.
...
#### FR-UG-06 — Employment Profile

**User Story:**
As a user, I want to set my employment profile so that the application shows me only the features and context selectors relevant to my working arrangement.

**Acceptance Criteria:**
- AC-UG-06-1: Each user account has an **Employment Type** setting with three options: `Employed` (day job only), `Sole Trader` (self-employed only), and `Both` (simultaneously holds employment and operates as a sole trader). Defaults to `Employed` for new accounts.
- AC-UG-06-2: The user can change their Employment Type at any time from their profile settings page.
- AC-UG-06-3: An Admin can view and change a user's Employment Type from the admin user detail page.
- AC-UG-06-4: **Employed** — Work logs do not show a Work Context selector (all effort is implicitly `Day Job`). The Finance module shows Personal and Work-Related categories only; Business category, Clients, and Invoicing are hidden.
- AC-UG-06-5: **Sole Trader** — Work logs do not show a Work Context selector (all effort is implicitly `Sole Trading`). The Finance module shows Personal, Work-Related, and Business categories; Clients and Invoicing are fully available.
- AC-UG-06-6: **Both** — Work logs require a Work Context selection on every entry (`Day Job` or `Sole Trading`). The Finance module shows all category types; transactions can optionally carry a Work Context tag; Clients and Invoicing are fully available.
- AC-UG-06-7: Changing Employment Type does not alter any historical work log entries or transactions — it only affects UI presentation going forward.
- AC-UG-06-8: The sidebar and module navigation adapt to the Employment Type — e.g., Clients and Invoicing nav items are hidden for `Employed` users even if Finance module access is enabled.
...
| 1.4     | 2026-03-06 | Tech Lead              | Added FR-UG-06 Employment Profile (Employed / Sole Trader / Both); updated FR-WL-01 with Work Context field (conditional on employment type); updated FR-WL-03 with Work Context filter; updated §5.2 overview. |
| 1.5     | 2026-03-16 | Tech Lead              | Added AC-UG-01-10 — Admin notification bell in top bar showing pending registration count; navigates to `/admin/users`; visible to ADMIN role only. Updated AC-UG-03-3 to document Admin sidebar nav link (shield icon, ADMIN-only). Added FR-T-05 — Kanban board view at `/tasks/board`; "New task" from board redirects back to `/tasks/board` via `?from=board` query param; cancel also respects return context. |

---

*End of Document — BRD-001 v1.5*