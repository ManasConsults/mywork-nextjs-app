...
| Date          | 2026-03-16                     |
| Related BRD   | BRD-001 v1.4 (Work Management) |
...
The Finance module extends *MyWork* with personal and business financial management capabilities for individuals. It serves three user profiles: **employees** (day job, no self-employment), **sole traders** (self-employed freelancers and consultants), and **dual-role users** who hold simultaneous employment and operate a sole trading business. The module enables users to track income and expenses across personal, work-related, and business categories appropriate to their profile; manage client relationships; raise and send invoices; and convert work log entries into billable invoice line items.
...
| BO-5 | ~~Deliver budget-vs-actual visibility to help individuals control personal and business spending.~~ *(removed — Budgets module descoped)* | ~~Medium~~ |
...
|---------------------------------|----------------|-------------------------------------------------------------------------------------|-----------|------------------------------|
| Employee (day job only)         | Primary User   | Track personal and work-related expenses; set budgets; simple financial picture.    | High      | Co-design, beta testing      |
| Sole Trader / Freelancer        | Primary User   | Manage personal and business finances; invoice clients; control budget.             | High      | Co-design, beta testing      |
| Dual-Role User (employee + ST)  | Primary User   | Separate day-job context from sole-trading context across logs, expenses, invoices. | High      | Co-design, beta testing      |
...
#### Persona A — Employee (from BRD-001, secondary for Finance)
- **Name:** Alex
- **Role:** Employed professional (no self-employment)
- **Goals:** Track personal and work-related expenses (home office, travel, professional subscriptions); set monthly budgets; see a simple financial picture.
...
#### Persona D — Sole Trader / Freelancer
- **Name:** Morgan
- **Role:** Independent Consultant / Freelancer (self-employed only, no employer)
...
#### Persona E — Dual-Role User (Employee + Sole Trader)
- **Name:** Sam
- **Role:** Full-time employed software developer who also takes on freelance contracts in evenings and weekends
- **Goals:** Keep day-job work logs clearly separate from freelance work; invoice freelance clients from logged hours; capture employed work-related expenses (rail travel, home-office proportion) separately from freelance business expenses; single tool covering both sides of their working life.
- **Pain Points:** Constantly mixing up which hours and receipts belong to the day job versus the freelance business; tax prep requires manually splitting data across two categories from a single spreadsheet.
...
- Three-tier category typing: Personal / Work-Related / Business (Business category only for Sole Trader and Both profiles).
- User-defined categories and subcategories.
- **Employment Profile awareness** (FR-UG-06): Finance module features and category types adapt to the user's Employment Type (`Employed`, `Sole Trader`, or `Both`).
- **Work Context tagging** on transactions for dual-role users (FR-FIN-19): tag transactions as `Day Job` or `Sole Trading` to enable separate reporting.
- Client management for invoicing (Sole Trader and Both profiles only).
- Invoice creation, editing, and status tracking (Draft / Sent / Paid / Overdue / Cancelled) (Sole Trader and Both profiles only).
- Invoice line items manually entered or auto-populated from Work Log entries (Sole Trader context only).
...
- Timesheet view: work log hours filtered by date range, billed/unbilled status, and Work Context (Sole Trader and Both profiles only).
- Financial reports: P&L by period, cash flow by period, expense breakdown by category, tax summary, unbilled hours; filterable by Work Context where applicable.
...
- AC-FIN-02-7: A transaction can be marked as recurring at creation time. The user selects a frequency (`Weekly`, `Fortnightly`, `Monthly`, `Quarterly`, `Annually`) and an optional end date. Recurring instances are generated automatically on page load (lazy evaluation) and displayed in the transaction list with a frequency badge. Recurring templates cannot be deleted without removing their generated series. Editing a recurring instance affects that occurrence only.
...
**Acceptance Criteria:**
- AC-FIN-05-1: A category requires: Name (max 100 chars) and Type. Available types depend on the user's Employment Profile (FR-UG-06): `Employed` users see `Personal` and `Work-Related`; `Sole Trader` and `Both` users additionally see `Business`.
- AC-FIN-05-2: Optional fields: Parent Category (for subcategories, one level deep only), Colour, Icon.
- AC-FIN-05-3: **Personal** — everyday personal spending (groceries, entertainment, rent). Not deductible for tax. Available to all Employment Profiles.
- AC-FIN-05-4: **Work-Related** — expenses incurred because of employment but not purely business (home office proportion, work travel, professional subscriptions). Partially or fully deductible depending on jurisdiction — flag for reporting only; the application does not calculate deductibility. Available to all Employment Profiles.
- AC-FIN-05-5: **Business** — sole trader business expenses and income (client invoices, professional fees, equipment used exclusively for business). Flagged as business for P&L and tax reporting. Available to `Sole Trader` and `Both` Employment Profiles only.
- AC-FIN-05-6: A default set of categories is provided on first use, tailored to the user's Employment Profile. Defaults include `Personal` and `Work-Related` categories for all profiles; `Business` defaults are added for `Sole Trader` and `Both` profiles.
...
- AC-FIN-05-8: If a user changes their Employment Profile from `Sole Trader` or `Both` to `Employed`, existing `Business` categories and their transactions are retained but hidden from the category picker. They remain visible in historical reports.

---

### 5.4 Budgets *(Removed — Descoped)*

> **REMOVED as of v1.5 (2026-03-16).** The Budgets module has been descoped from v1.0. The `Budget` model, `BudgetPeriod` enum, all related schemas, services, actions, and the `/finance/budgets/` route have been deleted. The `budgets` table was dropped via migration `20260314000000_remove_budget_module`. This section is retained for historical reference only.

---

#### FR-FIN-06 — Set a Budget *(Removed)*

~~**User Story:**
As a user, I want to set a budget for a category so that I can track whether I am staying within my spending limits.~~

**Acceptance Criteria:** *(all removed with module)*
...
- AC-FIN-07-2: Optional fields: Email, Phone, Address (multi-line), Default Hourly Rate, Notes (max 1000 chars). Currency is inherited from the creating user's profile currency (`session.user.currency`) at creation time and is not user-editable per client.
...
- AC-FIN-08-8: A `Cancelled` invoice can be permanently deleted. Deletion requires an inline confirmation prompt. Deletion cascades to all associated line items. Only `Cancelled` invoices may be deleted; all other statuses are protected.
...
### 5.9 Work Context on Transactions

**Overview:** For dual-role users (Employment Profile = Both), financial transactions can be tagged with a Work Context to attribute them to either the day job (Employed) or the sole trading business (Sole Trader). This enables segregated reporting for tax and budgeting purposes.

---

#### FR-FIN-19 — Work Context on Transactions

**User Story:**
As a dual-role user (employee and sole trader), I want to tag my financial transactions with a Work Context so that I can produce separate reports and expense summaries for my day job and my sole trading business.

**Acceptance Criteria:**
- AC-FIN-19-1: When the user's Employment Profile (FR-UG-06) is **Both**, a **Work Context** field is available on the transaction entry form. Values: `Day Job` (stored as `EMPLOYED`) and `Sole Trading` (stored as `SOLE_TRADER`). The field is optional — transactions without a Work Context tag are treated as unclassified.
- AC-FIN-19-2: When the user's Employment Profile is **Employed** or **Sole Trader**, the Work Context field is hidden on the transaction form (it is inferred from the profile and stored automatically for `Work-Related` and `Business` categories respectively).
- AC-FIN-19-3: The transaction list includes a Work Context filter (`All`, `Day Job`, `Sole Trading`, `Unclassified`) for **Both** profile users.
- AC-FIN-19-4: Financial reports (expense breakdown, tax summary) can be filtered by Work Context for **Both** profile users, enabling the user to generate an employed work-related expense report separately from a sole trader expense report.
- AC-FIN-19-5: Work Context is displayed as a badge on individual transaction rows in the list view when the user's profile is **Both**.
- AC-FIN-19-6: The Work Context field is available when editing an existing transaction.
...
| A-F5 | Users of the Finance module may be employees, sole traders, or a combination of both (see FR-UG-06). Multi-user business finance (shared P&L, multi-director companies) is out of scope in v1.0. |
...
| Budget            | *(Removed — Budgets module descoped in v1.5)* ~~A user-defined spending limit for a category over a defined period.~~ |
...
| 1.3     | 2026-03-06 | Tech Lead              | Added FR-UG-06 reference to employment profile (Employed / Sole Trader / Both); added Persona E (Dual-Role); updated Persona A; added §5.9 FR-FIN-19 Work Context on Transactions; updated FR-FIN-05 category gating; updated §4.1 scope; updated stakeholder table; updated A-F5; updated §1.1 purpose. |
| 1.4     | 2026-03-09 | Tech Lead              | Added AC-FIN-08-8 — `Cancelled` invoice permanent deletion (inline confirmation, cascades line items, restricted to CANCELLED status only); added AC-FIN-02-7 — recurring transaction creation (frequency, optional end date, lazy instance generation). |
| 1.5     | 2026-03-16 | Tech Lead              | Removed Budgets module (FR-FIN-06 descoped): deleted Budget model, BudgetPeriod enum, budget schema/service/action/route; dropped `budgets` table via migration `20260314000000_remove_budget_module`; removed budget setting from §4.1 scope; removed BO-5 budget objective; removed budget tracking sentence from §1.3; updated AC-FIN-07-2 — client currency now inherited from `session.user.currency` at creation time (migration `20260316000000_backfill_client_currency` backfilled existing records). |

---

*End of Document — BRD-002 v1.5*
