...
│   │   ├── board/
│   │   │   └── page.tsx             # Kanban board view — columns by status
│   │   ├── new/
│   │   │   └── page.tsx             # New task form; accepts ?from=board query param
│   │   ├── [id]/
│   │   │   └── page.tsx             # Task detail: logs, notes, linked to-dos
│   │   └── _components/             # TaskCard, TaskFilters, TaskForm (Client)
│   │                                # TaskForm accepts returnTo prop for post-save redirect
│   ├── work-logs/
...
| Shell (Client)        | AppShell (mobile state, hamburger, ThemeToggle, AdminNotificationBell) | Client       |
| Navigation (Client)   | Sidebar (desktop collapse + mobile drawer, AdminNavLink), NavContent | Client       |
...
| `setEmploymentTypeAction`   | User / Admin  | Set `employmentType` for a user (`EMPLOYED`, `SOLE_TRADER`, or `BOTH`). User can update their own; Admin can update any. |
...
erDiagram
    USER {
        ...
        enum employmentType "EMPLOYED|SOLE_TRADER|BOTH default EMPLOYED"
...
**Board return navigation:** When a new task is initiated from the Kanban board (`/tasks/board`), the "New task" link appends `?from=board` to the URL (`/tasks/new?from=board`). The `TaskForm` component reads this query param and stores it as a `returnTo` prop. On successful creation (and on cancel), the form redirects to `/tasks/board` instead of the default `/tasks`. This preserves board context without requiring server-side logic changes.
...
**Admin navigation link (Sidebar):** The Sidebar renders an Admin nav entry (shield icon, linking to `/admin/users`) only when `session.user.role === 'ADMIN'`. This is evaluated server-side in the RSC Sidebar and is not togglable from the client.

**Admin notification bell (AppShell):** When `session.user.role === 'ADMIN'`, the AppShell top bar renders a notification bell icon. The pending user count is queried server-side in `app/(app)/layout.tsx` before rendering the shell:

```typescript
// app/(app)/layout.tsx (after session guard)
const pendingCount = session.user.role === 'ADMIN'
  ? await prisma.user.count({ where: { isActive: false, rejectedAt: null } })
  : 0;
// pendingCount is passed as a prop to AppShell
```

The bell displays a red badge with the count when `pendingCount > 0`. Clicking the bell navigates to `/admin/users`. When `pendingCount === 0` or the user is not ADMIN, the bell is not rendered.

#### 10.3.1 Employment Profile in Session

`employmentType` (FR-UG-06) is included in the JWT payload and `session.user` alongside module flags:
...
callbacks: {
  session({ session, token }) {
    session.user.id = token.id;
    session.user.role = token.role;
    session.user.moduleWork = token.moduleWork;
    session.user.moduleFinance = token.moduleFinance;
    session.user.employmentType = token.employmentType; // 'EMPLOYED' | 'SOLE_TRADER' | 'BOTH'
    return session;
  }
}
```

Layout guards and server components use `session.user.employmentType` to conditionally render:
- Work Context selector in Work Log forms (only when `BOTH`)
- Business category option in Finance category forms (only when `SOLE_TRADER` or `BOTH`)
- Clients and Invoicing sidebar nav entries (only when `SOLE_TRADER` or `BOTH`)
- Timesheet billing UI (only when `SOLE_TRADER` or `BOTH`)

`setEmploymentTypeAction` in `lib/actions/profile.ts` (self-service) and `lib/actions/admin.ts` (admin override) updates the field and calls `revalidatePath` on the relevant page. Employment type change takes effect on next session refresh.
...
## ADR-006: FeedbackSubmission model — no soft-delete; cross-cutting platform concern
**Date:** 2026-03-22
**Status:** Accepted
**Context:** A new `FeedbackSubmission` model is required to support FR-FB-01 (user submission) and FR-FB-02 (admin management). Two questions required binding architectural decisions: (1) whether to apply the soft-delete pattern; and (2) whether the model belongs to an existing domain module or is a cross-cutting platform concern.
**Decision:**
1. No `deletedAt` column. Feedback submissions are append-only platform telemetry. Users have no delete UI (AC-FB-01-12: read-only list). Admins manage status transitions only (AC-FB-02-7/AC-FB-02-8). There is no GDPR-recovery requirement equivalent to Tasks/Notes/Achievements because submissions are not user-authored work product. If a submission must be removed for data-protection reasons, an Admin hard-delete action is acceptable and preferable to carrying Prisma middleware complexity on a model that will never surface a "restore" flow.
2. The module lives at `app/(app)/feedback/` (submission flow) and `app/(admin)/feedback/` (admin management). Service and action files go in `lib/services/feedback.service.ts` and `lib/actions/feedback.ts`. It must not be nested under any existing domain module (tasks, notes, finance) because it has no dependency on those entities.
**Consequences:** No soft-delete means no Prisma middleware extension for this model. Admin hard-delete must be implemented as an explicit, separately RBAC-guarded action and must be auditable (pino log at `warn` level including submissionId and adminUserId, never the description content). The `deletedAt`-based GDPR purge logic that will exist for Tasks/Notes/Achievements in v2 does not apply here; a separate Admin UI action covers any erasure request.
**Alternatives Considered:** Applying soft-delete for consistency — rejected because there is no corresponding restore flow, making it dead schema weight that increases Prisma middleware surface area with no product benefit.
...
| 1.4     | 2026-03-06 | Tech Lead         | Added `employmentType` enum to User ER model (`EMPLOYED \| SOLE_TRADER \| BOTH`, default `EMPLOYED`); added `setEmploymentTypeAction` to server actions table; added §10.3.1 Employment Profile in Session (JWT payload, layout guards, conditional feature rendering); updated BRD reference to v1.4. |
| 1.5     | 2026-03-16 | Tech Lead         | Added Kanban board route (`tasks/board/`) and `tasks/new/` to file structure (§5.1); added `returnTo` prop pattern on TaskForm and board return navigation note (§7.2); documented Admin nav link (shield icon, Sidebar, ADMIN-only) and Admin notification bell (AppShell, pending user count queried in layout, red badge) in §10.3.0; updated component classification table (§5.2) to reflect AdminNotificationBell and AdminNavLink; updated BRD reference to v1.5. |
| 1.6     | 2026-03-22 | Solution Architect | Added ADR-006: FeedbackSubmission model shape, no soft-delete decision, index strategy, enum placement, module boundary placement, and GDPR hard-delete approach. |

---

*End of Document — SAD-001 v1.6*
