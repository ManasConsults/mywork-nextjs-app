...
| Date          | 2026-03-16                          |
| Related BRD   | BRD-002 v1.5                        |
| Related SAD   | SAD-001 v1.4 (Work Management)      |
...
### 3.0.1 Employment Profile Feature Gate

`session.user.employmentType` (SAD-001 §10.3.1) drives feature visibility within the Finance module. The check occurs at the RSC layout and component level — no dedicated redirect; instead, features are conditionally rendered:

```typescript
// app/(app)/finance/layout.tsx — after module access check
// Pass employmentType to context so child RSCs can gate features:
const isSoleTrader = session.user.employmentType !== 'EMPLOYED'; // SOLE_TRADER or BOTH
const isDualRole   = session.user.employmentType === 'BOTH';
```

| Employment Type | Visible Finance Features |
|---|---|
| `EMPLOYED` | Accounts, Transactions (Personal + Work-Related categories), Reports (personal/work) |
| `SOLE_TRADER` | All of above + Business category, Clients, Invoicing, Timesheet Billing |
| `BOTH` | All features + Work Context tag on transactions and work logs, split reports by context |

Sidebar nav entries for **Clients** and **Invoicing** are hidden when `employmentType === 'EMPLOYED'`:
```typescript
{isSoleTrader && <FinanceClientsNavItem />}
{isSoleTrader && <FinanceInvoicingNavItem />}
```
...
├── transaction.service.ts   getTransactions, getTransactionById, createTransaction, updateTransaction, deleteTransaction
│                            getTransactionSummary (total in/out for a period)
│                            generateDueRecurrences (lazy generation of past-due recurring instances; idempotent; capped at 366 per series per call)
├── category.service.ts      getCategories, createCategory, updateCategory, deleteCategory
├── client.service.ts        getClients, getClientById, createClient(userId, data, currency), updateClient, archiveClient
│                            (currency param added — defaults to 'GBP'; inherits from session.user.currency via createClientAction)
...
│                            markInvoicePaid, cancelInvoice, revertToDraft
│                            deleteInvoice (restricted to CANCELLED status; cascades line items)
...
├── client.ts                createClientAction (reads session.user.currency; passes to createClient), updateClientAction, archiveClientAction
...
│                            deleteInvoiceAction (restricted to CANCELLED; inline confirmation enforced on client)
...
model Transaction {
  ...
  reference     String?
  date          DateTime
  workContext   WorkContext?                       // null = not applicable; set for EMPLOYED/SOLE_TRADER when user is BOTH (FR-FIN-19)
...
enum WorkContext {
  EMPLOYED      // Day Job
  SOLE_TRADER   // Sole Trading
}

enum EmploymentType {
  EMPLOYED      // Employee only
  SOLE_TRADER   // Self-employed only
  BOTH          // Employee and sole trader simultaneously
}
...
model WorkLog {
  ...
  workContext         WorkContext?                   // null when user has single employment type; required for BOTH users
  invoiceLineItem     InvoiceLineItem?               // back-relation
  client              Client?          @relation(fields: [clientId], references: [id])
}

// User model — add finance, module licensing, and employment fields:
model User {
  // ... existing fields ...
  ...
  employmentType      EmploymentType   @default(EMPLOYED) // FR-UG-06
  currency            String           @default("GBP")
...
-- Work context queries (BOTH employment type users):
CREATE INDEX idx_transactions_work_context ON transactions(user_id, work_context)
  WHERE work_context IS NOT NULL;
CREATE INDEX idx_work_logs_work_context ON work_logs(user_id, work_context)
  WHERE work_context IS NOT NULL;
```
...
CANCELLED → DELETE  (hard delete — restricted to CANCELLED; cascades all associated line items; client must show inline confirmation before calling deleteInvoiceAction)
```
...
| 1.3     | 2026-03-06 | Tech Lead          | Added `WorkContext` and `EmploymentType` enums; added `workContext WorkContext?` field to WorkLog extension and Transaction model (FR-FIN-19); added `employmentType EmploymentType` to User model extension (FR-UG-06); added §3.0.1 Employment Profile Feature Gate (feature gating logic, sidebar conditionals, session flags); added `idx_transactions_work_context` and `idx_work_logs_work_context` indexes; updated Related BRD to v1.3 and Related SAD to v1.4. |
| 1.4     | 2026-03-09 | Tech Lead          | Added `deleteInvoice` (CANCELLED-only, cascades line items) and `revertToDraft` to invoice.service.ts listing; added `deleteInvoiceAction` to invoice.ts actions listing; added CANCELLED → DELETE transition to §9.3; added `generateDueRecurrences` and `getTransactionById` to transaction.service.ts listing; updated Related BRD to v1.4. |
| 1.5     | 2026-03-16 | Tech Lead          | Removed Budgets module: deleted `budget.service.ts`, `budget.schema.ts`, `lib/actions/finance/budget.ts`; removed `Budget` model and `BudgetPeriod` enum from schema (§5.2); removed `budgets Budget[]` relation from Category and User models; removed `budgets/` route from §3.1 route structure; removed Budgets from EMPLOYED feature gate table (§3.0.1). Updated `createClient` signature to accept `currency` param (defaults to `'GBP'`); `createClientAction` now reads `session.user.currency` and passes it to `createClient`; updated service and action listings (§4.1, §4.3). Updated Related BRD to v1.5. |

---

*End of Document — SAD-002 v1.5*
