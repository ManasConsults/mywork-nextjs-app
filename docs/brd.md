...
#### FR-UG-05 — Module Licensing

**User Story:**
As an Admin, I want to control which modules (Work and/or Finance) each user has access to so that I can grant appropriate access during onboarding and adjust it over time.

**Acceptance Criteria:**
- AC-UG-05-1: Each user account has two module access flags: **Work** and **Finance**. Both default to `true` — all users have access to both modules unless explicitly restricted.
- AC-UG-05-2: The Admin panel displays a module access control for each user showing which modules are enabled, visible in the user list and user detail view.
- AC-UG-05-3: During user approval (transitioning from `PENDING` to `ACTIVE`), the Admin can set the module access flags before confirming approval. If not changed, both default to enabled.
- AC-UG-05-4: Admin can change a user's module access at any time from the admin panel, regardless of whether the user is active, pending, or rejected.
- AC-UG-05-5: Admin cannot remove their own module access.
- AC-UG-05-6: If a user has **Work** access disabled, they cannot access any route under the Work module (`/dashboard`, `/tasks`, `/work-logs`, `/achievements`, `/notes`, `/todo`); they are redirected to a "Module not available" page.
- AC-UG-05-7: If a user has **Finance** access disabled, they cannot access any route under the Finance module (`/finance/*`); they are redirected to a "Module not available" page.
- AC-UG-05-8: A user with neither module enabled can still log in but sees only a restricted landing page informing them to contact an Admin.
- AC-UG-05-9: Sidebar navigation entries for a module are hidden when the user does not have access to that module.
- AC-UG-05-10: Module access changes take effect immediately; the user's next page load or navigation will reflect the updated access (no re-login required; JWT is refreshed on next session check).
...
| A-10 | All new user accounts have both Work and Finance module access enabled by default (`moduleWork = true`, `moduleFinance = true`). Admins may restrict access to one or both modules at approval time or at any point thereafter. |
...
| Module License   | A per-user flag controlling access to a specific application module (Work or Finance). Both are enabled by default. |
...
| 1.2     | 2026-03-05 | Tech Lead              | GitHub OAuth active; mobile responsiveness; JWT callback fix.                                                                                                                           |
| 1.3     | 2026-03-06 | Tech Lead              | Added FR-UG-05 Module Licensing (Work/Finance access flags per user, admin-controlled, both enabled by default); added A-10; added Module License to glossary.                          |

---

*End of Document — BRD-001 v1.3*